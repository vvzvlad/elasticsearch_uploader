    ...
    foreach my $year ( @YEARS ) {
        foreach my $month ( @MONTHS ) {
            ...
            while ( $chtml =~ /href="($base_url(\d+?)\.html)"/gsm ) {
                ...
                print $cpage, "\t", $year, "\t", $month, "\t", $page_id, "\n";
            }
        }
    }
...
sub process_post(@) {
    ...
    if ( $html =~ /Site\.page = \{(.+?)\};/ ) {
        ...
        if ( $data ) {
            ...
            print "replycount: ", $data->{'replycount'} || 0, "\n";
            ...
            foreach ( @{$data->{'comments'}} ) {
                ...
                delete $_->{'suspended'};
                delete $_->{'siteroot'};
                delete $_->{'deleted_poster'};
                delete $_->{'leafclass'};
                delete $_->{'commenter_journal_base'};
                delete $_->{'noid'};
                delete $_->{'is_best'};
                delete $_->{'collapsed'};
                delete $_->{'statprefix'};
                ...
                if( length( $_->{'article'} // '' ) ) {
                    ...
                    #$_->{'article'} =~ s/\s+/ /gsm;
                    #$_->{'article'} =~ s/\t/ /gsm;
                    $_->{'article'} =~ s/(&laquo;|&raquo;|&quot;)/"/gsm;
                    $_->{'article'} =~ s/(&ndash;)/—/gsm;
                    $_->{'article'} =~ s/(&nbsp;)/ /gsm;
                    my $dc;
                    eval {
                        $dc = to_json( $_ );
    ...
    # Пишем контент
    open FP, ">>$DATA/$target_user\-$post_id\-content.json";
    my %content_data=('user' => $target_user, 'post_id' => $post_id, 'href' => $href, 'post_time' => $post_time, 'body' => $body, 'title' => $title, 'rpl' => $rpl,  'tags_string' => join(", ", @tags), 'links_string' => join(", ", @hrefs));
    print FP join("\t", to_json( \%content_data, { pretty => 1 } ) ), "\n";
    ...
    if ( $CM and ( $rpl != $cm ) ) {
        ...
        foreach $next ( 1..$last_page ) {
            ...
            if ( $data ) {
                ...
                $cm += $cm_on_page;
                $rpl = $data->{'replycount'};
                print "Reply coint : ", $data->{'replycount'}, "\n";
                print "COMMENTS: ", $cm_on_page, "\n";
    ...
    # Пишем комментарии
    if ( $CM ) {
        open FP, ">>$DATA/$target_user\-$post_id\-comments.json";
        #my %comments_data=('user' => $target_user, 'post_id' => $post_id, 'post_time' => $post_time, 'rpl' => $rpl,'cm' => $cm, 'comments' => @comments);
        #print FP join("\t", $target_user, $post_id, $post_time, $rpl, $cm, to_json( \@comments, { pretty => 0 } ) ), "\n";
        print FP join("\t", to_json( \@comments, { pretty => 0 } ) ), "\n";
        close FP;
    }
